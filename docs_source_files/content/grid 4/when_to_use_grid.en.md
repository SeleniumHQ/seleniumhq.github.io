---
title: "When to use Grid 4"
weight: 2
---

Generally speaking, there’s two reasons why you might want to use Grid 4.
...
Grid 4 is used to speed up the execution of a test pass by using
multiple machines to run tests in parallel. For example, if you have a suite of
100 tests, but you set up Grid 4 to support 4 different machines (VMs or
...
Grid 4 is also used to support running tests against multiple runtime
...
Selenium-Grid 4 receives each test-browser combination and assigns each test to
...
instances. When the suite runs, each test is passed to Grid 4 which
...
Grid 4 is very flexible. These two examples can be combined to allow
