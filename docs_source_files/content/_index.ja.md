...
Seleniumは、さまざまなブラウザーやプラットフォームでデスクトップWebおよびモバイルWebアプリケーションを検証するために使用される無料の（オープンソース）自動テストツールです。 Java、C＃、Pythonなどの複数のプログラミング言語を使用して、デスクトップおよびモバイルWebブラウザーの自動化を有効およびサポートするSeleniumテストスクリプトを作成できます。
