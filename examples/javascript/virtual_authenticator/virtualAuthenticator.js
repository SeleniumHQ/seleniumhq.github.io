...
(async function testVirtualOptions() {
...
(async function testCreateAuthenticator() {
...
(async function testRemoveAuthenticator() {
  let options = new VirtualAuthenticatorOptions()
  let driver = await new Builder().forBrowser('chrome').build();
  await driver.addVirtualAuthenticator(options)
...
(async function testCreateaAndAddResidentialKey() {
  let options = new VirtualAuthenticatorOptions()
...
(async function testAddResidentCredentialNotSupportedWhenAuthenticatorUsesU2FProtocol() {
  let options = new VirtualAuthenticatorOptions()
  options.setProtocol(VirtualAuthenticatorOptions.Protocol['U2F'])
  options.setHasResidentKey(true)
...
(async function testCreateAndAddNonResidentialKey() {
  let options = new VirtualAuthenticatorOptions()
  options.setProtocol(VirtualAuthenticatorOptions.Protocol['U2F'])
  options.setHasResidentKey(false)
...
(async function testRemoveAllCredentials() {
  let driver = await new Builder().forBrowser('chrome').build();
  await driver.addVirtualAuthenticator(new VirtualAuthenticatorOptions())
...
(async function testSetUserVerified() {
  let option = new VirtualAuthenticatorOptions()
  option.setIsUserVerified(true)
  
  assert(option.toDict()['isUserVerified'])
})();